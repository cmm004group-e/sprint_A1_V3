...
if(isset($_POST['register'])) {
   ...
   if($username == '')
    ...
    if($errMsg == ''){
        try {
            $stmt = $connect->prepare('INSERT INTO user_profile (firstname, lastname, email, username, password, jobtitle, company, job_desc, telephone, linkedin, twitter, instagram, facebook )VALUES (:firstname, :lastname, :email, :username, :password, :jobtitle, :company, :job_desc, :telephone, :linkedin, :twitter, :instagram, :facebook)');
            $stmt->execute(array(
                ':firstname' => $firstname,
                ':lastname' => $lastname,
                ':email' => $email,
